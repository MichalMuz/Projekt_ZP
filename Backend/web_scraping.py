    ...
    async def fetch_with_retry(self, session, url, headers, timeout=100000, retries=3):
        for _ in range(retries):
            try:
                ...
                print(f'Waiting time error during download {url}. Re-attempting ...')
...
async def get_number_of_pages(session, start_url, cache, pbar=None):
    ...
    if html_content:
        ...
        # If 'a' tags do not contain page number, try extracting from 'li' tags
        li_elements = html_content.find_all('li')
        for li in reversed(li_elements):
            # Check if the li element has a number and is not the ellipsis
            if li.get('aria-selected') == 'false' and li.get('class') != ['css-mgrgkd']:
                last_page_number = int(li.get_text(strip=True))
                if pbar:
                    pbar.set_postfix(pages=last_page_number)
                print('Number of pages to be analyzed:', last_page_number)
                return last_page_number
